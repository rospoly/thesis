...
\begin{table}[t]
	...
	\begin{tabular}{@{\extracolsep{2.3pt}}p{1.5cm}L{1.2cm}L{1.2cm}L{1.3cm}@{}}
		...
		\multicolumn{1}{c}{Format (bits)}& \multicolumn{1}{c}{Mantissa} & \multicolumn{1}{c}{Exponent} & \multicolumn{1}{c}{$\epsilon$}\\
		\midrule
...
%
Intuitively, in worst-case analysis we assume the error $\delta$, in Equation (\ref{standard}), has the worst magnitude possible. In other words, we make a conservative decision by setting $\delta=\epsilon$.
%
...
The natural question then would be whether we can \emph{implement} (on chip) the \emph{ideal} controller (from MATLAB), using low-precision formats while still guarantee the stability of our system.
%
