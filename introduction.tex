...
%
The immediate consequence is we always under-estimate the roundoff errors, thus we get non-rigorous (i.e. unsound) roundoff error bounds~\cite{glasserman2013monte, parker2000monte, herbie}.
%
...
%
In rigorous analysis of roundoff errors, which is the focus of this thesis, we assume the error $e$ in Equation (\ref{standard}) has the worst magnitude possible. In other words, we make a conservative choice by setting $e=\epsilon$.
%
...
We can use $\epsilon$ to make our program \emph{robust} to roundoff error~\cite{guardstable}.
%
...
%
Any unstable execution stems from the instability region, on the other hand, this is a conservative estimate because both stable and unstable executions can land in the instability region.
%
\begin{figure}[tb!]
	\begin{lstlisting}[frame=single, language=Python, escapeinside={(*}{*)}]
	x(*$\;\in\;$*)[0,10]
...
The most similar approach to the work described in this thesis is the roundoff error analysis called PrAn~\cite{probdaisy}; to the best of our knowledge, this is also the only work apart from this paper that presents a rigorous and general probabilistic roundoff error analysis. 
%
PrAn heavily relies on the worst-case error analysis implemented in Daisy~\cite{darulova2018daisy}, thus their approach inherits some of the limitations of worst-case analysis, in particular, the conservativeness of the error bounds.
%
The immediate consequence is the probabilistic roundoff errors from PrAn are always in the same order of magnitude of the worst-case errors from Daisy.
%
